...
### Identifiers are labelled as ours, not NPCI's

Every reference this prototype prints is generated locally, so nothing on a
receipt can be mistaken for something the UPI network returned:

| Surface | Field label | Value | Generated by |
| --- | --- | --- | --- |
| Payment receipt | **App payment reference** | `EPD-…` | `DemoPaytmService.charge` / seed data |
| Payment receipt | **Order reference** | `YYYYMMDD` + 6-digit sequence | `lib/ids.paytmStyleTxnId` |
| Settlement | bank reference | `EPD-STL-…` | `DemoPaytmService.settleNow` |
| Dynamic QR | `tr` parameter | `EPD…` | `services/paytm/upi.createTxnRef` |

The receipt field deliberately does **not** say "UPI transaction ID". A real UPI
transaction ID is issued by the network on authorisation, and this build never
contacts the network — so labelling our own string as one would misrepresent the
single thing a merchant uses to reconcile a disputed payment. When
`LivePaytmService` lands, that field takes the provider's identifier and the
label can change with it.
...
| Settlement to bank | **Simulated** | Bank reference (`EPD-STL-…`) is generated locally |
| Payment / settlement identifiers | **Ours, and labelled so** | "App payment reference", never "UPI transaction ID" |
...
A payment recorded through "Record a counter payment" on the **My QR** screen is
a prototype ledger entry. It is never presented as a confirmed bank settlement,
and the reference it produces is labelled as this app's own.
